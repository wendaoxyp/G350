...
//);
int FOSCSEL __attribute__((space(prog), address(0xF80006))) = 0xC0;
//_FOSCSEL(
...
//);
int FOSC __attribute__((space(prog), address(0xF80008))) = 0xFF;
//_FOSC(
//    POSCMOD_NONE &       // Primary Oscillator Configuration bits (Primary oscillator disabled)
//    OSCIOFNC_ON &        // CLKO Enable Configuration bit (CLKO output signal is active on the OSCO pin)
...
//);
int FPOR __attribute__((space(prog), address(0xF8000C))) = 0xF4;
//_FPOR(
...
//);
int FDS __attribute__((space(prog), address(0xF80010))) = 0x1F;
//_FDS(
...
int main() {
    ...
    UxBRG = 25;//9600((FCY + (BRG_DIV / 2 * BAUDRATE)) / BRG_DIV / BAUDRATE - 1); //BAUDRATEREG;

    UxMODEbits.UARTEN = 1; //enable uart
    UxSTAbits.UTXEN = 1; //enable receive
    TRISBbits.TRISB13=0;
    while (1) {
//        while (1) {
//            ClrWdt();
