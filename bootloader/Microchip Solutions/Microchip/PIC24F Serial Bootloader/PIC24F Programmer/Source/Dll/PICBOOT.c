...
HANDLE APIENTRY OpenPIC(LPSTR ComPort, DWORD BitRate, DWORD ReadTimeOut)
{
	...
	{
		ClearCommError(hComm,dummy1,dummy2);
		CloseHandle(hComm);
		return INVALID_HANDLE_VALUE;
	}
	else{
						...
						WORD NumOfBytes)
{
	...
	//Converted to one bulk write to better handle USB to Serial dongles
	if(!WriteFile(hComPort, OutData, i, &BytesWritten, NULL)) return ERROR_GEN_READWRITE;
...
INT APIENTRY WritePIC(HANDLE hComPort, PIC *pic, BYTE PacketData[])
{
	...
	if(pic->BootAddr*pic->BytesPerAddr % pic->BytesPerBlock) return ERROR_OFF_BOUNDRY;
	if(pic->BootDatLen % pic->BytesPerBlock) return ERROR_OFF_BOUNDRY2;
	...
	RetStatus = SendGetPacket(hComPort, OutData, DatCount, MAX_PACKET, pic->MaxRetrys);
