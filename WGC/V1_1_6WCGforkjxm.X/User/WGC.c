...
const uint8_t Sendstr[] = {0x01, 0x03, 0x00, 0x64, 0x00, 0x02, 0x85, 0xD4}; //获取压力的指令
...
void WGC_Init(void) {
    ...
    WGCIPbits.IP0 = 68;
    WGCPORT = 5070; //设置端口号
    WGCCON0bits.Send_60s = 5; //发送2分钟 mark1
    WGCCON1bits.Sample_5s = 12; //采集5秒
...
#ifdef DEBUG_Send
    RS485SendString1("GPRS DialingFail!!!");
#endif 
...
bool Sample_Task(void) {
    ...
    uint8_t Savstr[50] = {0x00}; //uart获取缓冲区数据01 03 04 3E F4 00 00 B6 29
    uint8_t cnt = 0; //计数
    ...
    do {
        cnt++;
        if (cnt > 5)
            return false; //循环5次后，还未获取到有效的压力值，直接返回false
        RS485SendString(Sendstr, 8);
        Delay1s(1); //延时1s 
...
bool WGCPCCop(void) {
    ...
    uint8_t tmp1[30]; //截取后的数据
    uint8_t tmp2[30]; //处理后要发送的数据
    uint8_t cnt;
    ...
    switch (funcode) {//根据功能码处理数据
        case 0x04://设置参数
        {
            switch (regadd) {//寄存器地址
                case 0://时间地址
                {
                    WGCTIME0bits.year = HCD(*(arg + 6));
                    WGCTIME0bits.month = HCD(*(arg + 7));
                    WGCTIME1bits.day = HCD(*(arg + 8));
                    WGCTIME1bits.hour = HCD(*(arg + 9));
                    WGCTIME2bits.min = HCD(*(arg + 10));
                    WGCTIME2bits.sec = HCD(*(arg + 11));
                    //                    bytes = 6;
                    break;
                }
                case 1://采集时间和发送时间
