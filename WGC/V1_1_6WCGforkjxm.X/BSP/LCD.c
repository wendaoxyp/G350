...
void TIME_SHOW(struct tm *time) {

    ClrNum();
    //    time->tm_year;//显示小时
    NUM_SHOW(time->tm_hour / 10, 5);
    NUM_SHOW(time->tm_hour % 10, 4);
    //    DP_SHOW(4);
    NUM_SHOW(time->tm_min / 10, 3);
    NUM_SHOW(time->tm_min % 10, 2);
    //    DP_SHOW(2);
    NUM_SHOW(time->tm_sec / 10, 1);
    NUM_SHOW(time->tm_sec % 10, 0);
}
