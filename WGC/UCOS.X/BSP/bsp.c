...
void BSP_Init(void) {
    ...
    Cur_ClkFrq = BSP_CPU_ClkFrq(); //获取晶振频率
...
static void Tmr_TickInit(void) {
    ...
    cnts = (Cur_ClkFrq / OS_TICKS_PER_SEC) - 1; /* Calaculate the number of timer ticks between interrupts  */
...
void OSCILLATOR_Initialize(void) {
    ...
    INTCON1bits.NSTDIS = 1;//中断嵌套关闭
    //    __builtin_write_OSCCONL(0x02);//使用外部晶振
    //    __builtin_write_OSCCONL(0x00); //使用外部晶振
    //    OSCCONbits.OSWEN = 1;
