...
//    }
//    return (uint16_t)( ucCRCHi|(ucCRCLo<<8));
//}
...
uint16_t CRC16_Check(uint8_t *CRC_Buf, uint8_t CRC_Leni) {
    ...
    *(CRC_Buf + CRC_Leni) = CRC_Sumx & 0x00ff;//自动添加CRC的数据结果
    *(CRC_Buf + CRC_Leni + 1) = CRC_Sumx >> 8;
    return (CRC_Sumx);
}
...
bool CheckIsRight(FP_CRC fp, uint8_t * dat, const uint8_t bytes) {
    uint16_t CRCtep = fp(dat, bytes); //CRC校验
