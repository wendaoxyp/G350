...
//OS_STK SLPStk[32];
//OS_STK PCCopStk[32];
OS_EVENT *sem_set_alarm, *sem_get_pressure, *sem_send_pressure, *sem_receive_pressure;
...
void Task2UseHL6528(void *p_arg) {
    ...
    while (1) {
        ...
        if (!HL6528_DialingIsERR()) {
            WGC_sta = 3; //GPRS发送数据
            OSTimeDlyHMSM(0, 0, 1, 0); //延时1s
            UART2_SendString("*89887#6688#sample*");
            OSTimeDlyHMSM(0, 0, 1, 0); //延时1s
        }
        HL6528_Close();
        WGC_sta = 0; //GPRS拨号
...
//void Task5TestI2C(void* parg) {
//    parg = parg;
//    struct tm Alatime;
//    uint32_t timestamp = 1499598934;
// CAT24C512_Init(); //应用程序初始化
//    INT8U err;
//    PCF8583_SetTime(&timestamp);
//    OSTimeDlyHMSM(0, 0, 0, 100);
//    PCF8583_SetStatusReg(0x05);
//    OSTimeDlyHMSM(0, 0, 0, 100);
//    Alatime.tm_hour = 11;
//    Alatime.tm_min = 15;
//    Alatime.tm_sec = 50;
//    PCF8583_SetAlarmTime(Alatime);
//    while (1) {
//
//        //        OSTimeDlyHMSM(0, 0, 50, 0);
//        if (!PORTBbits.RB13) {
//            OSTimeDlyHMSM(0, 0, 1, 0);
//            PCF8583_SetStatusReg(0x05); //清除PCF8583的定时报警标志
//        }
//
//        OSSemPend(sem2, 0, &err); //, 0, &err);
//        //        CAT24C512_Read(0, tmp2);
//        //        timestamp1 = PCF8583_ReadTime(); //1499598934
//        //        PCF8583_ReadStatus(tmp2);
