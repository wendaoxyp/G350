...
#define  LCD_SetLow(x,y) (LCDDATA##x &= ~(1 << y)) //Ä³Î»ÖÃ0 
#define  LCD_SetHigh(x,y) (LCDDATA##x |= (1 << y))  //Ä³Î»Çå1
...
#define CC_SHOW(x) ((x)?(LCD_SetHigh(12,7)):(LCD_SetLow(12,7)))
#define _h_SHOW(x) ((x)?(LCD_SetHigh(8,7)):(LCD_SetLow(8,7)))
#define m3_SHOW(x) ((x)?(LCD_SetHigh(4,7)):(LCD_SetLow(4,7)))
#define kPa_SHOW(x) ((x)?(LCD_SetHigh(0,7)):(LCD_SetLow(0,7)))


void LCD_Init(void);
void LCD_ShowTest(uint8_t, bool);
void BAT_SHOW(uint8_t bat);
void SIGN_SHOW(uint8_t sign);
void DP_SHOW(uint8_t dp_pos);
void NUMS_SHOW(uint32_t num);
