...
typedef  unsigned  long long   CPU_INT64U;                      /* 64-bit unsigned integer (ANSI89-EXTENSION)           */
typedef    signed  long long   CPU_INT64S;                      /* 64-bit   signed integer (ANSI89-EXTENSION)           */

typedef                 float  CPU_FP32;                        /* 32-bit floating point                                */
typedef            long double CPU_FP64;                        /* 64-bit floating point                                */
...
#define  CPU_CFG_ADDR_SIZE              CPU_WORD_SIZE_16        /* Defines CPU address word size  (in octets).          */
...
#define  CPU_CFG_STK_GROWTH       CPU_STK_GROWTH_LO_TO_HI       /* Defines CPU stack growth order (see Note #1).        */

#define  CPU_CFG_STK_ALIGN_BYTES  (sizeof(CPU_ALIGN))           /* Defines CPU stack alignment in bytes. (see Note #2). */

typedef  CPU_INT16U               CPU_STK;                      /* Defines CPU stack data type.                         */
...
typedef  CPU_INT16U                 CPU_SR;                     /* Defines   CPU status register size (see Note #3b).   */
...
#define  CPU_INT_DIS()         do { SRbits.IPL = 7; }  while (0)        /* Disable interrupts.                          */
#define  CPU_INT_EN()          do { SRbits.IPL = 0; }  while (0)        /* Enable  interrupts.                          */
#endif

#if     (CPU_CFG_CRITICAL_METHOD == CPU_CRITICAL_METHOD_STATUS_LOCAL)
                                                                        /* Save    CPU status word & disable interrupts.*/
#define  CPU_INT_DIS()         do { cpu_sr     = SR; \
                                    SRbits.IPL = 7;  } while (0)
#define  CPU_INT_EN()          do { SR = cpu_sr;     } while (0)        /* Restore CPU status word.                     */
#endif
