...
//    GSS0_OFF             // General Segment Code Protect (No Protection)
//);
 int FOSCSEL __attribute__((space(prog), address(0xF80006))) = 0x35;//0x34 ;32
//_FOSCSEL(
//    FNOSC_SOSC &         // Oscillator Select (Secondary oscillator (SOSC))
//    SOSCSRC_ANA &        // SOSC Source Type (Analog Mode for use with crystal)
//    LPRCSEL_LP &         // LPRC Oscillator Power and Accuracy (Low Power, Low Accuracy Mode)
//    IESO_OFF             // Internal External Switch Over bit (Internal External Switchover mode disabled (Two-speed Start-up disabled))
//);
 int FOSC __attribute__((space(prog), address(0xF80008))) = 0x2B ;
//_FOSC(
//    POSCMOD_NONE &       // Primary Oscillator Configuration bits (Primary oscillator disabled)
//    OSCIOFNC_OFF &       // CLKO Enable Configuration bit (CLKO output disabled)
//    POSCFREQ_LS &        // Primary Oscillator Frequency Range Configuration bits (Primary oscillator/external clock input frequency less than 100kHz)
...
//);
 int FWDT __attribute__((space(prog), address(0xF8000A))) = 0x5F ;//7f
//_FWDT(
//    WDTPS_PS32768 &      // Watchdog Timer Postscale Select bits (1:32768)
//    FWPSA_PR128 &        // WDT Prescaler bit (WDT prescaler ratio of 1:128)
...
//    WINDIS_OFF           // Windowed Watchdog Timer Disable bit (Standard WDT selected(windowed WDT disabled))
//);
 int FPOR __attribute__((space(prog), address(0xF8000C))) = 0x94;//96
//_FPOR(
//    BOREN_BOR0 &         // Brown-out Reset Enable bits (Brown-out Reset disabled in hardware, SBOREN bit disabled)
//    LVRCFG_OFF &         // Low Voltage Regulator Configuration bit (Low Voltage regulator is not available)
//    PWRTEN_OFF &         // Power-up Timer Enable bit (PWRT disabled)
//    I2C1SEL_PRI &        // Alternate I2C1 Pin Mapping bit (Use Default SCL1/SDA1 Pins For I2C1)
//    BORV_LPBOR &         // Brown-out Reset Voltage bits (Low-power Brown-Out Reset occurs around 2.0V)
//    MCLRE_ON             // MCLR Pin Enable bit (RA5 input pin disabled,MCLR pin enabled)
//);
 int FICD __attribute__((space(prog), address(0xF8000E))) = 0x82 ;
//_FICD(
//    ICS_PGx2             // ICD Pin Placement Select bits (EMUC/EMUD share PGC2/PGD2)
//);
 int FDS __attribute__((space(prog), address(0xF80010))) = 0x5F ;//1f
 //_FDS(
//    DSWDTPS_DSWDTPSF &   // Deep Sleep Watchdog Timer Postscale Select bits (1:2,147,483,648 (25.7 Days))
//    DSWDTOSC_LPRC &      // DSWDT Reference Clock Select bit (DSWDT uses Low Power RC Oscillator (LPRC))
