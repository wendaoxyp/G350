...
#define REG3_BYTES 34               //发送缓冲区数据字节数 differentn forA2 46-12减少了工况纵总量和工况瞬间量
...
#define UDP_A2_LENGTH 42            //42
#define UDP_13_LENGTH 36            //36
...
//#define SettingSampleTime 6         //开始获取瞬间量的时间
...
struct FLag_Arg {
    ...
    u16 LowPoweralarmFlag : 1; //报警上线标志位  
//    u16 LowPressurealarmFlag : 1; //低压报警
//    u16 HighPressurealarmFlag : 1; //高压报警
//    u16 LowTemplarmFlag : 1; //低温报警
//    u16 HighTempalarmFlag : 1; //高温报警
    u16 AlarmGetDataFlag : 1; //报警采集
    u16 LowPowerGetDataFlag : 1; //低电压标志位
    u16 TestGetDataFlag : 1; //其他数据标志位（测试）
    u16 TestOnlieFlag : 1; //测试上线标志位（测试）
    u16 SleepExitFlag : 1; //休眠模式退出标志
    u16 CheckRTCCFlag : 1; //校验RTCb标志位    
    u16 TestFlag : 1; //调试标志位 
    u16 T1CountFlag : 1; //T1开始计时
    u16 T1CountLastFlag : 1; //T1开始计时
    u16 PulseTimeFlag : 1; //脉冲的时间已经获取到了    
};
...
//    GPRS_No_Ready = 0x80, Data_No_Receive = 0x81, Data_Error_Check_Sum = 0x82, Data_Error_MpId = 0x83,
...
extern enum sendDataType {
    SampleData = 0, LowPowerData = 1, TestData = 2, AlarmData = 3/*温度和压力的数据*/
} SDT;
