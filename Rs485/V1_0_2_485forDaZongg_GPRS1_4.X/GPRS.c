...
bool CopUDPTask(const u8 sendDat, const u8 goLineCount, const u8 sendCount) {
    ...
    u8 FailCount = 0, sendCnt = 0, cnt = 0, getbytes = 0, tmp = 0;
    //    u8 mi_id_Tmp[8] = {0, 0, 0, 0, 0, 0, 0, 0};
    //    u8 Data_Type, bytes;
    //    u8 tmprtcc[7] = {0, 0, 0, 0, 0, 0, 0};
    bool sta = false;
    ...
    while (FailCount++ < goLineCount) {//2次上线
        ...
        {
            Statusbits.staGPRS = GPRS_No_Ready;
            Close_UART2(); //关闭串口2
            return false;
        }
        ...
        ClrWdt(); //清除看门狗        
        Statusbits.staGPRS = GPRS_Dialing;
        if (LKIsHigh(GPRS_ON_TIME)) {//判断LK在60s内是否上线        
            Statusbits.staGPRS = GPRS_Send_Data;
            sendCnt = 0; //清除发送次数
            while (sendCnt++ < sendCount) {
                while (BufferRead_UART2(&tmp)); //清除GPRS接收缓冲区
                UDPSendData(sendDat); //, offsetAdd);
                Delay1s(3); //延时4s 必须要等待2s后数据返回
            }
            ...
            if (getbytes = UARTIsGet(BufferRead_UART2, receiveBuffer, 55, UDPHead, 2)) { //获取数据并校验数据     //如果有接收到数据 
                if (receiveBuffer[getbytes - 2] == Sum_Check256(receiveBuffer, getbytes - 2)) { //判断校验和是否正确53 42   获取到的字节数减对于1位和校验吗                  
                    if (CheckSrting(receiveBuffer + 2, miotArgs.mi_ID, 8)) {
                        Statusbits.staGPRS = DUPDataChange(receiveBuffer) ? RData_Change_OK : RData_Change_Fail; //更新Miot的数据
                        if (FLagArgbits.TestFlag) Delay1s(1); //如果是调试模式的话，延时500ms，让上位机读取
                        gTimeCount.timeGprs = gTimeCount.timeMp; //确保每次发送的数据正常
                        if (sendDat == SampleData)//如果是采集数据的话，采集数据通道不清除
                            EmptyRegister1(); //清除缓冲数据
                        gAlarmZeroLock = (miotArgs.timealarm) ? 0 : 1; //如果报警值不为零的话，清除锁存位   
                        sta = true;
                        break; //退出拨号
                    } else
                        Statusbits.staGPRS = RData_Error_EID;
                } else
                    Statusbits.staGPRS = RData_Error_Check_Sum;
            } else
                Statusbits.staGPRS = RData_No_Receive;
            //        }
        } else
            Statusbits.staGPRS = GPRS_Dialing_Time_Over;
        //        if (sta)//如果获取成功，那么直接退出
        //            break;
    }
    if (!sta && (FailCount > goLineCount + 1))//如果上线次数超过了，表示为置1，最后一次数字+1，然后上线OK
