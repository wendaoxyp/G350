...
void TIM2_Configuration(void) {//用于485通讯延时
    ...
    PR2 = 12500; //800ms   1/（4/256*1000000）*?=1500/1000ms 【100ms】?=6250
...
void GetUnixTime_Char(u8* timeptr, u8* output) {
    ...
    times_l = mktime(&gmt) - 28800; //28800s(8小时)减去修正仪的GMT+8时间
    ptr = &times_l;
...
void GetUnixTime_YYYYMMDD(const u8* input, u8 *output) {
    ...
    CopyDat((u8*) & tmp, input, 4); //待测试
    gmt = gmtime(&tmp);
    *output = DCH((gmt->tm_year / 100) + 19);
    *(output + 1) = DCH(gmt->tm_year % 100);
