...
u8 CopFun(u8*dat, u8 source) {//
    ...
    switch (*(dat + 1)) {//读取数据包
        ...
        case GetMIOTArg://读取MIOT参数
        {
            ...
            tmp[1] = 0X04; //赋值功能码                   
            tmp[2] = REG2_BYTES; //GetSize(reg_add, arg_num); //赋值字节数
            add_base = tmp[2];
        ...
        case SetMIOTArg://设置Miot1601参数
        {
            ...
            Register2Write(dat + add_base, 0, REG2_BYTES); //设置数据全部写入miot1601里面
            Refresh_RTCC(); //刷新RTCC
            Refresh_Pulse(); //刷新脉冲数
...
static void TCP_03_Reply(u8 source, u8 start_ID, u8 nums) {//恢复采集数据
    ...
    if (FLagArgbits.TestOnlieFlag && (nums == 1)) //避免资源二次调用，导致设备复位
        ...
        if (nums == 1) {
            GetValueTask(SampleData); //采集一次数据
...
static void TCP_32_Reply(void) {//获取GPRS参数
    ...
    u16 CRC_Result = 0; //CRC校验结果
    //    Open_UART2();
    Register2Read(receiveBuffer, REG2_Arg_Postion[mi_add], 1); //赋值Miot1601地址
...
void UDPSendData(u8 packetType) {//, u8 OffSet, const u8 SampleCnt) {
    ...
    switch (packetType) {
        case SampleData:
        {
            ...
            for (cnt = 0; cnt < sampCnt; cnt++) {//组合数据包 //赋值数据包的数据            
                ...
                if (sampCnt - 1 == cnt) {
                    ...
                    TCP_SendData(GPRS, sendBuffer3, 46); //使用中断发送给数据，避免数据丢失
                    Delay100ms(5); //避免数据发送过快导致GPRS下线
                }
            }
            break;
        }
        ...
        case TestData:
        {
            ...
            StartTime4();
            IFS1bits.T4IF = 1; //开启定时器4的中断
            while (IFS1bits.T4IF)StartTime4(); //等待获取采集数据
            //            GetRegister1(REG3_BUF2, sendBuffer2, (cnt + startCnt) % REG3_BUF1_CNT); //获取数据
            GetRegister1(REG3_BUF2, sendBuffer2, 1); //获取数据 different 仅一个通道的数据
            GetUnixTime_Char(sendBuffer2, sendBuffer3); //46=A2(42)+时间戳(4)//获取时间戳 08 16 1 1 0 0 0 时间容易溢出
            PacketData2A2(sendBuffer3 + 4, sendBuffer2); //获取协议 赋值给数据包  A2参数+时间戳18=数据头(14)+地址位在A2中的位置(4)
            sumCheck += Sum_Check256(sendBuffer3, 46); //获取和校验
            sendBuffer3[46] = sumCheck;
        ...
        while (bytes < *(source_dat + 13) + 14) {
            ...
            switch (Data_Type) {
                ...
                {// Save Period (seconds) 时间的表示方式不对                    
                    ...
                    Register2Write((source_dat + bytes + 1), REG2_Arg_Postion[mi_tmp], 2);
                    bytes += 3;
                    break;
                }
                case 0x01:
                {// Send Period (seconds)                   
                    ...
                    Register2Write((source_dat + bytes + 1), REG2_Arg_Postion[mi_tgprs], 2);
                    bytes += 3;
                    break;
                }
                case 0x02:
                {// Alarm Period (seconds)
                    ...
                    Register2Write((source_dat + bytes + 1), REG2_Arg_Postion[mi_talarm], 2);
                    bytes += 3;
                    break;
                }
                case 0x03:
                {// High Pressure Alarm 需要加判断
                    Register2Write((source_dat + bytes + 1), REG2_Arg_Postion[mi_hp_alarm], 4);
                    bytes += 5;
                    break;
                }
                case 0x04:
                {// Low Pressure Alarm 需要加判断
                    Register2Write((source_dat + bytes + 1), REG2_Arg_Postion[mi_lp_alarm], 4);
                    bytes += 5;
                    break;
                }
                case 0x05:
                {// High Temperature Alarm 需要加判断
                    Register2Write((source_dat + bytes + 1), REG2_Arg_Postion[mi_ht_alarm], 4);
                    bytes += 5;
                    break;
                }
                case 0x06:
                {// Low Temperature Alarm 需要加判断
                    Register2Write((source_dat + bytes + 1), REG2_Arg_Postion[mi_lt_alarm], 4);
                    bytes += 5;
                    break;
                }
                case 0x07:
                {// 时间 需要加判断
                    ...
                    Register2Write(tmprtcc, REG2_Arg_Postion[mi_year], 7);
                    Refresh_RTCC();
                    bytes += 5;
                    break;
                }
                case 0x08:
                {// 低压报警 需要加判断
                    Register2Write((source_dat + bytes + 1), REG2_Arg_Postion[mi_lv_alarm], 4); //将服务器发送的数据写入MIOT的参数表中
