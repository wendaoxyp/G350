...
void __attribute__((__interrupt__, __auto_psv__, __shadow__)) _T2Interrupt(void) {
    //    while (!U1STAbits.RIDLE);//等待485通讯空闲
    PC_Cop();
    T2CONbits.TON = 0;
//    u16 tmp = TMR2;
    TMR2 = 0;
...
void __attribute__((interrupt, no_auto_psv)) _INT1Interrupt(void) {
    OSCChange(0x00); //将主频切换回来
    OpenRS485(); //开启485通讯
    FLagArgbits.TestFlag = 1; //进入调试模式，避免时间过短进入睡眠模式
    if (!T2CONbits.TON) {
        T2CONbits.TON = 1; //开启定时器2进入参数设置
//        PR2 = 1562;
    }
//    IEC1bits.INT1IE = 0; //关闭本中断 
    IFS1bits.INT1IF = 0; //清楚标志位
}
...
void __attribute__((interrupt, no_auto_psv)) _DefaultInterrupt(void) {
    ...
    Register2Write(rtcc_time, REG2_Arg_Postion[mi_year], 7);
    SaveMiotArg();
    //    FM25L64B_Write(BOR_Pulse, (u8*) (&gPulseCount), 4); //保存脉冲总量
