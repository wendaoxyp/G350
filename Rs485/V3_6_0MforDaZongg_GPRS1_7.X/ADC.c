...
void ADC_Init(void) {
    ADC0_IN;
    ANSAbits.ANSA2 = 1; //AD1CHS配置模拟IO口
    AD1CON1 |= (1 << 10); //12bits
    //带参考
    ...
    AD1CON3bits.ADCS = 0b00111111; //64Tcy
    AD1CON3bits.SAMC = 0b11111; //31
    AD1CON2bits.ALTS = 0; //始终对一个通道采集
    AD1CON2bits.PVCFG = 0b10; //1.024*2
...
static u16 GetVoltage(void) {
    ...
    while (!IFS0bits.AD1IF); // conversion done?//卡死在此处
    ...
    AD1CON1bits.ADON = 0;
    return (u16) (ADCValue >> 4);
    //    return a; //0.8642578125=3.54/2^12*1000
}

//AD采集电压转化系数 3.05690375 

void GetMuilVoltage(float * ptr_f) {
    u8 count;
    u32 voltage_l = 0;
    //    float result=0;
    for (count = 0; count < 4; count++) {//均值滤波
        voltage_l += GetVoltage();
    }
    //    return (float) (0.00015284 * voltage_l);
    *ptr_f = (float) (3.056 * (voltage_l >> 2) / 2000);
    //    return result; //转化为电压值3.05690375 /2=1.528451875
}
