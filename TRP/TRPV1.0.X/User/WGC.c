...
const uint8_t PRES[] = {0x01, 0x03, 0x04};
...
const uint8_t TestByte[] = {
    0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, '\r', '\n',
    0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, '\r', '\n',
    0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, '\r', '\n',
    0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, '\r', '\n',
    0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, '\r', '\n',
...
//const uint8_t Code_06_Rec = {0x57, 0x47, 0x43, 0x06, 0x00, 0xB7, 0x0C};
//const uint8_t Code_06_Fail = {0x57, 0x47, 0x43, 0x06, 0x01, 0x76, 0xCC};
...
//公司IP地址和端口：183.246.74.68:5070
//阿里云IP地址和端口：139.196.122.159：5080
void WGC_Init(void) {
    ...
    WGCCON2bits.Debug1 = WGCCON2bits.Debug2 = 0; //关闭E41
...
//        LED_Off();
        ...
        for (cnt = 0; cnt < 15; cnt++) {
            UART2_SendString2(TestByte, 120); //发送传输数据
            Delay1s(2); //延时1s
        }
        if (WGCCON2bits.Debug2)//测试点2 GPRS
            RS485SendString1("GPRS ONline Now!!!");
    } else {
...
bool Sample_Task(void) {
    ...
    //    WGCPCCop()
    //    ClearBuffer(BufferRead_UART1);//避免数据过多导致缓冲区满接收失败
    do {
        cnt++;
        if (cnt > 3)
            return false; //循环5次后，还未获取到有效的压力值，直接返回false 添加错误提示
        RS485SendString(GetPressureInt, 8);
        Delay100ms(5); //延时2s 必须小于1s 避免其他指令获取数据导致数据丢失
...
uint32_t CalaAlarmTime(void) {
    ...
    if (WGCCON2bits.Period_FLAG) {
        ...
        WGCCON3bits.count = WGCCON0bits.Send_60s * 12 / WGCCON1bits.Sample_5s; //计算采集的次数 当WGCCON1bits.Sample_5s=0时触发数学错误
...
bool WGCPCCop(void) {
    ...
    if (UARTDataIsRight(&stcp) && CheckIsRight(CRC16_Check, tmp1, 4)) {
        //根据寄存器地址、数量、数据长度和数据,判断进行设置数据
        CopyDat(tmp2, tmp1, 4); //数据帧头+功能码  
        tmp2[4] = RegAdd[*(tmp1 + 4)+*(tmp1 + 5)] - RegAdd[*(tmp1 + 4)];
        if (tmp2[4] > RegAdd[6])//避免数据溢出错误
            tmp2[4] = RegAdd[6];
        switch (*(tmp1 + 3)) {
                //            case 0x04:
                //            {
                //                //加阈值保护                
                //                // *send: 57 47 43 04 00 06 17 12 10 12 10 20 00 03 00 05 03 B7 F6 4A 44 12 CE 4E 09   
                //                // *rec:57 47 43 04 11 17 12 10 12 10 20 00 03 00 05 03 B7 F6 4A 44 12 CE 6B A1
                //                if ((((uint16_t) tmp1[12] << 8) + tmp1[13]) < 3) {//阈值处理
                //                    tmp1[12] = 0;
                //                    tmp1[13] = 3;
                //                } else if ((((uint16_t) tmp1[12] << 8) + tmp1[13]) > 1440) {
                //                    tmp1[12] = 0x05;
                //                    tmp1[13] = 0xA0;
                //                }
                //                if (((uint16_t) tmp1[14] << 8) + tmp1[15] < 2) {//阈值处理
                //                    tmp1[15] = 0;
                //                    tmp1[15] = 2;
                //                } else if (((uint16_t) tmp1[14] << 8) + tmp1[15] > 17280) {
                //                    tmp1[14] = 0x43;
                //                    tmp1[15] = 0x80;
                //                }
                ...
                //                RTC_Refresh_Time();
                //                WGCCON2bits.Period_FLAG = 1; //重新计算RTCC 导致输出下次定时的时间
                //                IFS3bits.RTCIF = 1;
            ...
            case 0x06:
            {
                ...
                if (WGCCON2bits.Send_FLAG) {
                    ...
                    rec[4] = 0;
                    WGCCON2bits.Send_FLAG = 1; //进入GPRS模式
                }
                CRC16_Check(rec, 5);
                RS485SendString(rec, 7);
                break;
            }
        }
        return true;
    ...
    //    stcp.SourceData = tmp;
    //    stcp.Source_len = UARTGetData(BufferRead_UART1, stcp.SourceData, 50); //获取mark
    stcp.CheckData = (uint8_t*) PRES;
    stcp.Check_len = 3;
    stcp.GetData = tmp1;
    stcp.Get_len = 30;
    stcp.Real_len = 0;

    if (UARTDataIsRight(&stcp) && CheckIsRight(CRC16_Check, tmp1, 7)) {//获取压力数据        
        CharToFloat(&f_p, tmp1 + 3);
        ShowPressure(); //显示压力值
    }
    return false;
}
        ...
        case 0x04://设置参数
        {
            switch (regadd) {//寄存器地址
                ...
                case 1://采集时间和发送时间
                {
                    ...
                    DataIsVaild(WGCCON0bits.Send_60s, 3, 1440);
                    WGCCON1bits.Sample_5s = ((uint16_t)*(arg + 14))+*(arg + 15); //直接BCD码
                    DataIsVaild(WGCCON1bits.Sample_5s, 2, 17280);
                    break;
                }
                case 2://ID号
                {
