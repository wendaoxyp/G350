...
static bool HL6528SendCmd(const uint8_t *ss, const uint8_t* cs, const uint8_t* cs1, const uint8_t csl, const uint8_t cnt) {
    ...
    ClearBuffer(BufferRead_UART2); //清除UART2的接收数据
    do {
        cnt_tmp += 1;
        UART2_SendString(ss); //发送AT指令                               
        Delay1s(1); //(0, 0, 3, 0); //延时1s
        U2Rx4Byte();
        stcp.SourceData = tmp;
        stcp.Source_len = UARTGetData(BufferRead_UART2, stcp.SourceData, 80); //获取mark
...
uint8_t HL6528_DialingIsERR(void) {
    ...
    DTR = 0;
    Delay1s(3);
    if (HL6528SendCmd(D1, OK, NULL, 2, 6)) {
        if (HL6528SendCmd(CPIN, READY, NULL, 8, 6)) {
            if (HL6528SendCmd(CREG, _5, _1, 2, 20)) {//设置网络模式
                if (HL6528SendCmd(KCNXCFG, OK, NULL, 2, 6)) {
                    //HL6528_GetIP_Port(); //刷新IP地址和端口号
                    if (HL6528SendCmd(KTCPCFG1, OK, NULL, 2, 6)) {
                        if (HL6528SendCmd(KTCPSTART, CONNECT, NULL, 5, 6)) {
                            sta = 0;
                        } else
                            sta = 6; //未开启TCP，
                    } else
...
void HL6528_Close(uint8_t err_code) {
    uint8_t tmp[20];
    StrCmp stcp;
    ClearBuffer(BufferRead_UART2);
    if (!err_code) {
