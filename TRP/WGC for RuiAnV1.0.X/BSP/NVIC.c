...
void __attribute__((interrupt, no_auto_psv)) _ISR _RTCCInterrupt(void) {
    //    uint8_t tmp = 0;
    //    WGCCON2bits.Send_FLAG = 1; //添加时间计算避免时间上的误差
    //    RS485SendString
    /*    tmp = CalaAlarmTime(); //计算下次闹钟的时间
        SetNextAlarm(tmp); //设置下次闹钟时间
        if (WGCCON2bits.Debug1) {//如果测试点1 时间输出测试
            RS485SendString1("WGC\r\n");
            printnums(tmp);
            if (WGCCON2bits.Sample_FLAG) {
                //        WGCCON2bits.Sample_FLAG = 0;
                RS485SendString1("Sample\r\n");
            }
            if (WGCCON2bits.Send_FLAG) {
                //        WGCCON2bits.Send_FLAG = 0;
                RS485SendString1("Send\r\n");
            }
        }*/
    //第三方认证不需要
...
void __attribute__((__interrupt__, __auto_psv__, __shadow__)) _T2Interrupt(void) {
    ...
    if (WGCCON2bits.Sample_FLAG) {
        ...
        //        RS485SendString1("\r\nStart Sample!!! count:");
        //        //            Delay100ms(3);
        //        printnums(WGCCNTbits.Sample_CNT++);
        //        NUMS_SHOW(WGCCNTbits.Sample_CNT, 0);
        //#endif
        if (Sample_Task()) {
            LED_Toggle1(2, 4);
            ...
            WGCCON2bits.Sample_FLAG = 0; //等同与定时器2标志位
        }
    }*/
    //第三方认证不需要
...
//    StopTMR1(); //关闭定时器 
    //    WGCCON2bits.PC_Cop_FLAG = 0;
    //    if (WGCCON2bits.PC_Cop_FLAG) {
    //        if (cop_cnt > 1) {//接收数据1s后，在指令
    //            cop_cnt = 0;
    ////    U1Rx4Byte(); //接收最后小于4字节的数据，
    //    WGCPCCop();
    //if (WGCPCCop())
    //ClearBuffer(UART1_ReadBuf); //处理用户通讯交互任务
    //            WGCCON2bits.PC_Cop_FLAG = 0;
    //        } else
    //            cop_cnt += 1;
    //    }
...
void __attribute__((interrupt, no_auto_psv)) _U1RXInterrupt(void) {
    uint8_t cnt = 0;
    LED_On();
//    WGCCON2bits.PC_Cop_FLAG = 1; //表示PC正在和设备通讯
    //    StartTMR1(); //定时接收数据
    for (cnt = 0; cnt < 4; cnt++)
