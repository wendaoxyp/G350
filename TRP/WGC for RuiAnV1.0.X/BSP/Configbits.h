...
/*睡眠模式需要以下几点：
 * 1、关闭BOREN，开启DSSWEN和LPCFG
 * 2、硬件上Vcap需要连接10uF电容
 * 3、将RCONbits.RETEN=1，然后DSEN=1，Sleep
 */


// CONFIG4
#pragma config DSWDTPS = DSWDTPS1F    // Deep Sleep Watchdog Timer Postscale Select bits->1:68719476736 (25.7 Days)
#pragma config DSSWEN = ON    // DSEN Bit Enable->Deep Sleep operation is always disabled
...
#pragma config VBTBOR = ON    // VBAT BOR enable bit->VBAT BOR enabled  注意功耗测试
#pragma config BOREN = ON    // Brown-out Reset Enable->Brown-out Reset Enable  onite
...
// CONFIG2
#pragma config POSCMD = XT    // Primary Oscillator Select->Primary Oscillator Disabled
#pragma config IESO = ON    // Internal External Switchover->Enabled
#pragma config IOL1WAY = ON    // IOLOCK One-Way Set Enable bit->Once set, the IOLOCK bit cannot be cleared
#pragma config FNOSC = FRCDIV//PRIPLL    // Initial Oscillator Select->Fast RC Oscillator with PLL module (FRCPLL) 注意功耗测试
